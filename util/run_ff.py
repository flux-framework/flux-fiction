...
def first_submit_epoch(trace_path: Path) -> float:
    with trace_path.open(newline="") as f:
        ...
        for row in reader:
            ...
            if submit:
                # Match Flux Fiction's trace parser: naive datetime minus
                # naive epoch, preserving sub-second precision.
...
def main() -> int:
    ...
    if args.otel:
        ...
        bridge_extra_lines = [
            ...
            f"cd {shell_quote(ff_root / 'src')}",
            "export PYTHONPATH="
            + shell_quote(f"{ff_root / 'src'}:${{PYTHONPATH:-}}"),
    ...
    try:
        if bridge_cmd is not None:
            ...
            bridge_env["PYTHONPATH"] = (
                f"{ff_root / 'src'}:{bridge_env.get('PYTHONPATH', '')}"
            )
